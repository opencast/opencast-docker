...
Opencast comes in different distributions. For each of the official distributions, there is a specific Docker image. Each version is tagged. For example, the full image name containing the `admin` distribution at version `9.4` is `quay.io/opencast/admin:9.4`. Leaving the version out will install the latest one.
